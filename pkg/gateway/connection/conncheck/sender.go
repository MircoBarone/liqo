...
func NewSender(ctx context.Context, opts *Options, clusterID string, cancel func(), conn *net.UDPConn, ip string, interfaceID int) (*Sender, error) {
	pip := net.ParseIP(ip)
	if pip == nil {
...
func (s *Sender) SendPing() error {
	msgOut := Msg{ClusterID: s.clusterID, InterfaceID: s.interfaceID, MsgType: PING, TimeStamp: time.Now()}
